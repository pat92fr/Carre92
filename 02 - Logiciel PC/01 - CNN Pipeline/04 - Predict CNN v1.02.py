...
# load model
model = load_model('04 - CNN/model.h5')
...
# load images
list = os.listdir('05 - Test Set/')
for i in list:
    print("05 - Test Set/"+i)
    image = cv2.imread("05 - Test Set/"+i,cv2.IMREAD_COLOR)
    ...
    # to gray
    image_smoothed= cv2.blur(im_resized,(3,3))
    ...
    for layer_name, layer_activation in zip(layer_names, activations): # Displays the feature maps
        ...
        plt.savefig('05 - Predict/'+i+'_'+layer_name+'.png')
        #plt.show()
        #plt.pause(1)
        plt.close('all')
    ...
    plt.savefig('05 - Predict/'+i+'_overlay.png')
    #plt.show()
    plt.pause(1)
    #plt.close('all')
