...
fifo_shape = (depth*skip,picture_initial_height,picture_initial_width,1)
...
while(capture.isOpened()):
    ...
    # FIFO
    fifo[:-1] = fifo[1:]; fifo[-1] = frame
    assert(fifo.shape == fifo_shape)
    x = fifo[0:depth*skip:skip]
    assert(x.shape == sequence_shape)

    #prediction
    y = model.predict(x.reshape(1,depth,picture_initial_height,picture_initial_width,1))
    dir = y.item(0)
    if True:
        print(str(dir))
        plt.clf()
        plt.imshow(x[-1].reshape(picture_initial_height,picture_initial_width), cmap = 'gray' )
        plt.axvline(x=(dir+1.0)/2.0*picture_initial_width,linewidth=2, label='DIR',color='r')
        plt.axvline(x=int(picture_initial_width/2),linewidth=2)
