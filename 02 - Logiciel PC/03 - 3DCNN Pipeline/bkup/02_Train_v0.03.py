...
def build_sequence_dataset(X, Y, depth):
...
depth = 10
Xsequence,Ysequence = build_sequence_dataset(X, Y, depth)
...
for c in param_conv_layers:
    ...
    if c[0] == 'maxpooling2D':
        model.add(MaxPooling2D(pool_size=c[1]))
    if c[0] == 'avgpooling2D':
        model.add(AveragePooling2D(pool_size=c[1]))
    if c[0] == 'maxpooling3D':
        model.add(MaxPooling3D(pool_size=c[1]))
    if c[0] == 'avgpooling3D':
        model.add(AveragePooling3D(pool_size=c[1]))
    if c[0] == 'dropout':
