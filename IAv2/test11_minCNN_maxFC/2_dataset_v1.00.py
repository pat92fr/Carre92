...
height_start = 10 # low offset from bottom
height_end = height_start+crop_height # high offset from bottom
...
X_train = np.zeros((width*crop_height*1,m*3))
...
for i in range(0,m):
    ...
    im_croped = image_gray[height-height_end:height-height_start,:width]
    assert(im_croped.shape == (crop_height,width))
    ...
    for j in range(0,3):
        ...
        assert(x.shape == (crop_height*width,1))
        #plt.imshow(x.reshape((crop_height, width)))
        #plt.show()
        ...
        #debug
        plt.clf()
        #plt.imshow(X_train[:,3*i+j].reshape((crop_height, width, 3)))
