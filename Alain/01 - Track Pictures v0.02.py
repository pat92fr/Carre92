...
class DataSet:
  ...
  bandeHauteurBas=0.18 # 18% du bas ==> PRB
  bandeHauteurMilieu=0.53 # 53% du bas ==> PRM
...
# =============================================================================
        ...
        if rrW.size > 0 : # bande blanche visible sur l'image
          ...
          if random.randint(0, 1) == 1:
            img[ccBL, rrBL]=np.add( np.multiply(img[ccBL, rrBL],0.05), np.multiply(self.colorBlack,0.95)) # blend
            img[ccBR, rrBR]=np.add( np.multiply(img[ccBR, rrBR],0.05), np.multiply(self.colorBlack,0.95)) # blend
          # Metadonnées :
          # a angle de la ligne
