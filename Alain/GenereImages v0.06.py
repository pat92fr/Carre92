...
def GenereImagePerturbee(params):
  ...
  ### Modif Alain : erreur lors de la conversion de l'image en ubyte
  #  skimage.io.imsave(filename, skimage.util.img_as_ubyte(img6))
  skimage.io.imsave(filename, img6)
  ### Fin modif Alain
...
class DataSet:
  ...
  ###### Modifs alain
  #bandeHauteurBas=-0.05 # 18% du bas ==> PRB
  #bandeHauteurHaut=0.45 # 53% du bas ==> PRM
  bandeHauteurBas=0.2 # 18% du bas ==> PRB
  bandeHauteurMilieu=0.6 # 53% du bas ==> PRM
  ###### Fin modifs Alain
  ...
  def AddLignes(self):
    ...
    yBandeBasse=int(self.camera.sizeY*(1.-self.bandeHauteurBas))
    yBandeMilieu=int(self.camera.sizeY*(1.-self.bandeHauteurMilieu))
    ###### Fin modifs Alain
    for angle in range(self.minAngle, self.maxAngle+self.stepAngle, self.stepAngle):
      for largeurRelat in range(self.minLargeurRel, self.maxLargeurRel+self.stepLargeurRel, self.stepLargeurRel):
        ...
        if rrW.size > 0 : # bande blanche visible sur l'image
          ...
          ###### Modif Alain : les images de fond doivent être de la taille des images générées
          # img=skimage.io.imread(imgname)
          img2=skimage.io.imread(imgname)
          img=skimage.transform.resize(img2, [self.camera.sizeY, self.camera.sizeX])
          ###### fin modif Alain
          picture_count += 1 # inc
          ...
          ###### Debug Alain : affichage des lignes 
          # r=np.array([imgMin[1]-1, imgMin[1]+1, imgPF[1]]) # y
          # c=np.array([imgMin[0], imgMin[0], imgPF[0]])
          # [rrWl,ccWl]=skimage.draw.polygon(c,r, [self.camera.sizeX, self.camera.sizeY])
          # img[ccWl, rrWl]=(0,0,255)
          # img[ccBas, rrBas]=(0,0,255)
          # img[ccMil, rrMil]=(0,0,255)
          # debug_xbas=(1.0+largeurRelatBas)*self.camera.sizeX/2
          # debug_xmil=(1.0+largeurRelatMilieu)*self.camera.sizeX/2
          # r=np.array([yBandeBasse, yBandeBasse, yBandeMilieu, yBandeMilieu])
          # c=np.array([debug_xbas-5, debug_xbas+5, debug_xmil+5, debug_xmil-5]) # y
          # [rrWl,ccWl]=skimage.draw.polygon(c,r, [self.camera.sizeX, self.camera.sizeY])
          # img[ccWl, rrWl]=(0,255,0)
          ###### End Debug Alain
...
if __name__ == '__main__':
  ...
  camera=Camera(40., [35, 60.], [25., 40.]) ##  coté milieu (60,25) coin bas (40,20) en 4/3
