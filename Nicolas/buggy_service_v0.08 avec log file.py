...
class buggyServer:
    ...
    def buggyservice(self):
		...
		# Ouverture d'un fichier dont le nom contient la date et l'heure
		day_str = time.asctime()
		day_str = day_str.replace(' ','_')
		logFile = open('log_' + day_str + '.csv', 'w')
        ...
        while True:
            ...
            while(True):
				...
				# Enregistrement des lidars
				logFile.write(serial_line + ";{:d};".format(self.direction).encode('ascii') + "{:d};".format(self.throttle).encode('ascii') + "{:d}\r\n".format(self.mode).encode('ascii'))
