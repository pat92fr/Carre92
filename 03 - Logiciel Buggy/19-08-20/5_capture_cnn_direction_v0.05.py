...
camera = cv2.VideoCapture(gstreamer_pipeline(capture_width=1280, capture_height=720, display_width=width, display_height=height, framerate=60, flip_method=2), cv2.CAP_GSTREAMER)
###camera.start()

# loop
### 64 fps with serial without predict without imshow (asynch)
### 51 fps with imshow and serial without predict (asynch)

### 54-55 fps with serial & predict without imshow (synch)
### 45-50 fps with serial & predict & imshow & line (synch)
...
try:
  while(True):
    ...
    x = np.array([x_1, x_2])
    y = model.predict(x)
    y_1,y_2 = y.item(0),y.item(1)
    #print(str(y_1))
    #print(str(y_2))

    #view
    if False:
      cv2.line(image,(int((y_1+1.0)/2.0*width),int((height-height_start_1-crop_height/2))),(int((y_2+1.0)/2.0*width),int((height-height_end_2+crop_height/2))),(255,0,0),5)
