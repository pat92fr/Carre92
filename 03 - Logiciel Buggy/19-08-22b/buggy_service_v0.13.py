...
def load_configuration_file():
    ...
    for line in file:
        ...
        if fields[0] == "SPEED":
            param_cruise_speed =  int(fields[1])
    file.close()
...
class buggyHandler(asyncore.dispatcher_with_send):
    ...
    def handle_read(self):
        ...
        if data:
            ...
            if (res[0] == "GET_API"):
                resp = "GET_API;param_kp;param_ki;param_kd;param_cruise_speed"
                print(resp)
            ...
            if (res[0] == "GET"):
                resp = "GET;" + str(param_kp) + ";" + str(param_ki) + ";"  + str(param_kd) + ";"  + str(param_cruise_speed)
                print(resp)
                self.send(resp.encode("utf-8"))
                print('Response sent.')
                return
            ...
            if (res[0] == "SET"):
                ...
                param_cruise_speed = float(res[4])
                command_id = 10
                return        
...
class buggyServer(asyncore.dispatcher):
    ...
    def process(self):
        ...
        # based on LIDARs    
            ...
            #print("error: " + str(error) + " window: " + str(self.pid_wall_following.integral_error))
        ...
        # throttle control
        if self.running:
            self.throttle = self.cruise_speed
            # stupid speed regulation
            if abs(self.pid_wall_following.integral_error)>250.0:
                self.throttle -= 1
            if abs(self.pid_wall_following.integral_error)>500.0:
                self.throttle -= 1
            if abs(self.pid_wall_following.integral_error)>750.0:
                self.throttle -= 1
            if abs(self.pid_wall_following.integral_error)>950.0:
                self.throttle -= 1
            self.mode = 1
        else:
            self.throttle = 128
    ...
    def reload_settings(self):
        ...
        self.cruise_speed = param_cruise_speed
