...
global param_direction_kp
global param_direction_ki
global param_direction_kd
...
def load_configuration_file():
    ...
    for line in file:
        ...
        if fields[0] == "WALL_FOLLOWING":
            param_direction_kp = float(fields[1])
            param_direction_ki = float(fields[2])
            param_direction_kd = float(fields[3])
        if fields[0] == "SPEED":
    ...
    def handle_read(self):
        ...
        if data:
            ...
            if (res[0] == CMD_GET_API):
                resp = "GET_API;direction_kp;direction_ki;direction_kd;speed_kp;speed_ki;speed_kd;speed_kff;min_speed;max_speed;k_speed;positional_error_threshold;dual_rate;direction_trim"
                print(resp)
            ...
            if (res[0] == CMD_GET):
                resp = "GET;" + str(param_direction_kp) + ";" + str(param_direction_ki) + ";"  + str(param_direction_kd) + ";"  + str(param_speed_kp) + ";" + str(param_speed_ki) + ";"  + str(param_speed_kd) + ";"  + str(param_speed_kff)  + ";" + str(param_min_speed) + ";"  + str(param_max_speed) + ";"  + str(param_k_speed) + ";"  + str(param_positional_error_threshold) + ";"  + str(param_dual_rate) + ";"  + str(param_direction_trim)
                print(resp)
                self.send(resp.encode("utf-8"))
                print('Response sent.')
                return
            ...
            if (res[0] == CMD_SET):
                param_direction_kp = float(res[1])
                param_direction_ki = float(res[2])
                param_direction_kd = float(res[3])
                param_speed_kp = float(res[4])
                param_speed_ki = float(res[5])
                param_speed_kd = float(res[6])
                param_speed_kff = float(res[7])
                param_min_speed = float(res[8])
                param_max_speed = float(res[9])
                param_k_speed = float(res[10])
                param_positional_error_threshold = int(res[11])
                param_dual_rate = float(res[12])
                param_direction_trim = int(res[13])
...
class main_controller():
    ...
    def reload_settings(self):
        ...
        global param_dual_rate
        global param_direction_trim        
        self.pid_wall_following.kp = param_direction_kp
        self.pid_wall_following.ki = param_direction_ki
        self.pid_wall_following.kd = param_direction_kd
